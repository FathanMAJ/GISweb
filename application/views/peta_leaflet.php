...
$.getJSON("<?=base_url()?>assets/RSU.geojson",function(data){                  
    var ratIcon = L.icon({                      
        iconUrl: '<?=base_url()?>assets/HOSPITAL.png',                      
        iconSize: [8,12]  
                });  
                L.geoJson(data,{                      
                pointToLayer: function(feature,latlng){                      
...
$.getJSON("<?=base_url()?>assets/puskesmas.geojson",function(data){                  
    var ratIcon = L.icon({                     
        iconUrl: '<?=base_url()?>assets/ULER.png',                      
        iconSize: [12,10]  
                });  
                L.geoJson(data,{  
                pointToLayer: function(feature,latlng){                      
...
$.getJSON("<?=base_url()?>assets/poliklinik.geojson",function(data){                  
    var ratIcon = L.icon({                      
        iconUrl: '<?=base_url()?>assets/PLUS.png',                      
        iconSize: [12,10]  
                });  
                L.geoJson(data,{                      
                pointToLayer: function(feature,latlng){                      
...
$.getJSON("<?=base_url()?>/assets/sungai.geojson",function(kode){                  
    L.geoJson( kode, {                  
        style: function(feature){                      
            ...
            if ( kode < 2 ) color = "#00FFFF";                      
            else if ( kode > 0 ) color = "#00FFFF";                      
            else color = "#00FFFF"; // no data                      
            return { color: "#999", weight: 1, color: color, fillOpacity: .8 };                  },                  onEachFeature: function( feature, layer ){                      layer.bindPopup  
                    ()  
                }  
            }).addTo(sungai);  }); 
